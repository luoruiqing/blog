...
CDN注入依赖的方式, 需要取决于CDN的响应速度, 若CDN响应较慢, 自己的页面打开速度就会受到影响, 若自身服务器不存在速度上的瓶颈, 还是推荐使用自定义分块, 否则稳定性和快速渲染是最重要的
...
大概长这样, 可以明确看到每个包的大小, 以及各个模块的分布情况, 自行百度了解
...
1. 打包后用浏览器打开 `report.html` 文件, 例如: **\<项目\>/dist/report.html文件**
2. 执行上面的命令, 然后浏览器打开 [http://0.0.0.0:8000/report.html](http://0.0.0.0:8000/report.html)
...
这个问题是 更改过入口 或 使用多入口开发(多页应用) 或 未使用默认配置导致的, 修改自定义的抽块依赖配置:
...
此时通过预览页可以看到 **element-ui** 被单独拆到了一个包内, 但是 `vendors` 这个包不见了, 打开浏览器页面还是正常的, 先忽略这个问题. 
...
根据上面的配置, 可以看到每个块的 `name` 都是要写入到 `pages.[PAGE].chunks` 中, 这里写个简单的程序兼容一下, 方便后续维护
...
这里看到 index 这个块依然很大, 包括 `echarts`、 `highcharts`、 `mockjs` 等等, 那我们就把所有的块都拆分出来, 这是我个人的配置:
...
const SPLIT_CHUNKS = {
    ...
    'chunk-fe-cube-ui': { 'test': /[\\/]node_modules[\\/]fe-cube-ui/ },
...
这次的基本拆分的非常细了, `index` 块的大小已经从 `4.72MB` 缩减到了 `660.04KB`, 质的提升, 同时我们看一下加载

![img]({{ site.baseurl }}/img/in-post/development/javaScript/webpack/build/07.png " ")

虽然看上去访问的时间不太长, 时间线基本上都是很快响应, 主要是因为访问的是本地的网络和文件, 速度较快, 如果换成客户机访问会导致页面等待最大的包而白屏, 这里的 **echarts** 明显过大, 想要解决这个问题的办法就是继续拆包

#### 子包拆分

打开预览页发现 **echarts** 的 **地图JSON数据** 依然占据了太多的空间, 那我们需要针对 **echarts** 进行再切割
...
代码中 `priority` 是根据下标大小决定优先级的, 所以 **chunk-echarts** 在 **chunk-echarts-map** 的上面, 优先级低, 这样才能正确分包, 所以使用这段代码一定要注意顺序.
...
如果是有依赖的工程, 不建议将 **[CHUNK].test** 用方法的形式, 会有大量的日志吐出到命令行, 会崩溃的.
